...
void ControlDependence::getControlDependencies(Function &F, PostDominatorTree &PDT) {
  ...
  for (depmap::iterator mi = controlDeps_.begin(), me = controlDeps_.end(); 
      mi != me; ++mi) {
    ...
    for (std::set<BasicBlock *>::iterator si = curSet.begin(), 
        se = curSet.end(); si != se; ++si) {
...
void ControlDependence::toDot(std::string name) const {
 ...
 (*out) << "label=\"CDG for \'" << name << "\' function \";\n";
  ...
  for (ControlDependence::depmap::const_iterator i = controlDeps_.begin(), 
...
void ControlDependence::toDot(std::string name, depmap map) const {
 ...
 // std::ofstream out;
  ...
  // create an edge for each dependency
  for (ControlDependence::depmap::const_iterator i = map.begin(), 
      ei = map.end(); i != ei; ++i) {
    BasicBlock *tail;
    tail = i->first;
